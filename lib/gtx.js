...
function wrapGrunt(grunt) {
	...
	gtx.finalise = function (cfg) {
		...
		var addTask = function (tasks, data) {
			return function (pluginName, targetName, targetData) {
				if (arguments.length < 2) {
					throw (new Error('expected at least 2 arguments'));
				}
				if (arguments.length === 2) {
		...
		var runTask = function(tasks) {
			return function (plugin, target) {
				if (arguments.length < 1) {
					throw (new Error('expected at least 1 argument'));
				}
				if (arguments.length === 2) {
		...
		var callFunc = function (tasks, data) {
		...
		var doLog = function (tasks, data) {
			return function (value) {
				lib.pushUnique(tasks, gtx.log(value, data.id));
			};
		};

		var pushGroup = function (groupName) {
			lib.pushHash(groups, groupName, alias);
		};

		// add macro instances
		id = Object.keys(taskMap)[i];
		while(id) {
			data = taskMap[id];
			all.push(data);
			...
			var add = addTask(tasks, data);
			var run = runTask(tasks);
			var tag = addGroup();
			var call = callFunc(tasks, data);
			var log = doLog(tasks, data);
			...
			// execute
			macro.func.apply(null, [context, data.id, data]);
