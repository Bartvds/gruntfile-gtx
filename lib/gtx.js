...
function wrapGrunt(grunt) {
	...
	function addTaskList(name, label, tasks, concurrentLimit) {
		...
		if (concurrentLimit > 1) {
			...
			gtx.configFor('concurrent', concurrentID, cfg);
	...
	// call multiple times but run only once
	gtx.runOnce = function (task) {
		var id = lib.getNameUID('gtx-once');
		grunt.registerTask(id, function () {
			lib.splitify(arguments).forEach(function (task) {
				if (hasRun.indexOf(task) < 0) {
					grunt.task.run();
				}
			});
		});
	...
	gtx.finalise = function (cfg) {
		...
		if (useAutoLoad) {
			...
			if (grunt.util.file.exists('./tasks')) {
	...
	gtx.registerMultiTaskt = grunt.registerMultiTaskt;
	...
	gtx.readJSON = function (paths) {
		...
		forEach(arguments, function (arg) {
			if (typeof arg === 'string') {
				ret = grunt.util._.extend(ret, grunt.file.readJSON(arg));
			}
			else {
				ret = grunt.util._.extend(ret, arg);
			}
		});
		return ret;
	};
	...
	gtx.readYAML = function (paths) {
		...
		forEach(arguments, function (arg) {
			if (typeof arg === 'string') {
				grunt.util._.extend(ret, grunt.file.readYAML(arg));
			}
			else {
				grunt.util._.extend(ret, arg);
			}
