...
function wrapGrunt(grunt) {
	...
	function addAlias(name, body) {
		console.log('addAlias: ' + name + ' ' + typeof body);
		createdAliases.push({name: name, tasks: body});
		grunt.registerTask(name, body);
	}
	...
	gtx.finalise = function (cfg) {
		...
		forEach(taskMap, function (data) {
			...
			var callFunc = function (func) {
				...
				grunt.log.writeln('callFunc ' + func);
	...
	gtx.multi = grunt.registerMultiTaskt;
